...
class PANNIncompModel():
	'''isochoric Free energy -- Physics-augmented neural network'''
	
	def __init__(self,parameterPath:str, nnParameterFile: str, onlyInvar = 0) -> None:
		
		weightsHL_mainPath,biasesHL,weightsHL_skipCon,weightsOL_mainPath,biasOL,weightsOL_skipCon = ssv.load_from_file(parameterPath,nnParameterFile)
		...
		self.weightsHL_skipCon = weightsHL_skipCon # list of tf.Tensor, one element per HL except for first HL
		...
		self.weightsOL_skipCon = tf.reshape(weightsOL_skipCon,shape=(weightsOL_skipCon.shape[0],)) # tf.Tensor

		# consistency check
		if len(self.weightsHL_mainPath) != (len(self.biasesHL)) or len(self.weightsHL_mainPath)  != (len(self.weightsHL_skipCon)+1):
	...
	def printParams(self):
		...
		for iHL in range(nHL):
			...
			if iHL > 0:
				print(self.weightsHL_skipCon[iHL-1].shape)
				print(self.weightsHL_skipCon[iHL-1].numpy())

		print(f'---------- OUTPUT LAYER ----------')
		iData=0
		for datael in [self.weightsOL_mainPath,self.biasOL,self.weightsOL_skipCon ]:
			print(datael.shape)
			print(datael.numpy())
			iData +=1
	...
	def psi_NN(self,F:tf.Tensor) -> tf.Tensor:
		...
		for iHL in range(1,self.nHL):						
			f_iHL = self.activFu(tf.einsum('nN,bn->bN',self.weightsHL_mainPath[iHL],f_iHL) +tf.einsum('iN,bi->bN',self.weightsHL_skipCon[iHL-1],invars) + self.biasesHL[iHL])
		
		# OL
		psi_NN = tf.einsum('n,bn->b',self.weightsOL_mainPath,f_iHL) + tf.einsum('i,bi->b',self.weightsOL_skipCon,invars)+self.biasOL
